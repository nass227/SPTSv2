...
def read_result(result_path, lexicons, pairs, match_dist_thres, gt_folder, lexicon_type):
    ...
    if 'totaltext' or 'ctw' in result_path:
        results_save = []
        for i in range(len(results)):
            idx = results[i]['image_id']
            if i == 0 or idx != results[i-1]['image_id']:
                if i != 0:
                    results_save += tmp
                tmp = [results[i]]
            else:
                for j in range(len(tmp)):
                    assert tmp[j]['image_id'] == results[i]['image_id']
                    if ed.eval(results[i]['rec'], tmp[j]['rec'])<1 and Point((tmp[j]['polys'][0][0],tmp[j]['polys'][0][1])).distance(Point((results[i]['polys'][0][0],results[i]['polys'][0][1])))<30:
                        flag = False
                        if     tmp[j]['score'] > results[i]['score']:
                            pass
                        else:
                            tmp[j] = results[i]
                    else:
...
def evaluate(results, gts, conf_thres, IS_WORDSPOTTING):
    ...
    for result in results:
        ...
        if IS_WORDSPOTTING:
            if include_in_dictionary(pred_rec) == False: 
                continue
        if gt_recs[idxmin] == "###" or gt_dontcare[idxmin] == True or gt_recs[idxmin] == '':
            continue
...
def main(args):
    ...
    if 'totaltext' in args.result_path.lower():
        ...
        lexicon_type = 1
    elif 'inversetext' in args.result_path.lower():
        gt_folder = 'evaluation/gt/gt_inversetext'; IS_WORDSPOTTING = True
        ...
        lexicon_paths = ['', 'evaluation/lexicons/ctw1500/ctw1500_lexicon.txt', ]
        pair_paths = ['', 'evaluation/lexicons/ctw1500/ctw1500_pair_list.txt', ]
        lexicon_type = 1
    ...
    print('Evaluating')
    conf_thres_list = np.arange(0.85, 0.95, 0.001)
    ...
    for conf_thres in tqdm(conf_thres_list):
        precision, recall, hmean, pgt, ngt, ndet = evaluate(
            ...
            IS_WORDSPOTTING = IS_WORDSPOTTING
        )
